...
/*
    ...
    for ($i=0; $i<count($menu); $i++){
        $v .= '<li class="menu_emp" onclick="cambiar('.$i.',false);" ondblclick="cambiar2('.$i.',false);" ><input type="radio" name="rdo" id="chk'.$i.'"><label for="chk'.$i.'"><div><a>'.  ucwords(strtolower($menu[$i]['Razon'])).'</a></div></label><ul id="lista'.$i.'"  >';
        $m1 = $menu[$i]['SubMenu'];
        $suscripcion = $menu[$i]['Suscripcion'];
        $x=5;
        foreach ($m1 as $key1 => $value1) {
            $v .= '<li class="submenu_emp"  onclick="cambiar('.$x.',true);" ondblclick="cambiar2('.$x.',true);" ><input type="radio" id="rb'.$i.$j.'" name="chk'.$i.'"><label for="rb'.$i.$j.'"><div><a>'.$key1.'</a></div></label><ul id="lista_'.$x.'">';
            $m2 = $value1;
    ...
    $v = '<div class="mnuempresa" id="firstpane">';
    $j=0;
    for ($i=0; $i<count($menu); $i++){
        $v .= '<p class="menu_head" >'.  ucwords(strtolower($menu[$i]['Razon'])).'</p>';
        $m1 = $menu[$i]['SubMenu'];
        $suscripcion = $menu[$i]['Suscripcion'];
        $v .= '<div class="menu_body">';
        foreach ($m1 as $key1 => $value1) {
            $v .= '<p class="menu_head" >'.$key1.'</p>';
            $m2 = $value1;
             $v .= '<div class="menu_body">';
            foreach ($m2 as $key2 => $value2) {
                $v .= '<div class="submenu_body" ><a onclick=enviaVista("'.$value2.'","PanelB","");>'.$key2.'</a></div>';
