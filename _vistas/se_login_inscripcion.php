...
$enlace = "../_vistas/se_login_inscripcion.php";
$ConxGestionDC = ConGestionDC();
//$ConexSisEmp = conexSis_Emp("LOCALHOST", "fri");
$cod;
// $tUrlAPOWL = "http://owlgroup.org";
...
function P_Login(){
    ...
    /*$sql = "SELECT Codigo,CONCAT(Nombres,' ',Apellidos) as User,Estado,CtaSuscripcion, UMiembro FROM sys_usuarios "
              ."WHERE Usuario = '".$email."' AND Contrasena = '".$Contrasena."' ";
    */
    W($sql);
    ...
    W($can);
    if ($can == 0 ){ WE(MsgE('! Error de Usuario y/o Contraseña')); }
    ...
    $_SESSION['Usuario']['string'] = $rg['Codigo'];
    $_SESSION['User']['string'] = $rg['User'];
    //verificar uso
    $_SESSION['Empresa']['string']  = $CodEmpresa;
    $_SESSION['Nom_bd']['string']   = $Nom_BD;
    $_SESSION['Servidor']['string'] = $Servidor;
    //////////////////////////////////////
    
    $_SESSION['CtaSuscripcion'] = $rg['CtaSuscripcion'];
    $_SESSION['UMiembro']['int'] = $rg['UMiembro'];
